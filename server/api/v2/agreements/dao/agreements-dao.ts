				...
				if(agreement){
					...
					if(tenantId == IDUser || role == "admin"){
						...
						if(agreementStatusloi == "accepted" && agreementStatusTa == "pending"){
							Agreements.penalty(paymentIdLoi, body.reason).then((res) => {
...
agreementsSchema.static('penalty', (id:string, data:Object):Promise<any> => {
	return new Promise((resolve:Function, reject:Function) => {
		if (!_.isString(id)) {
			return reject(new TypeError('Id is not a valid string.'));
		}
			...
			.exec((err, payment) => {
				...
				if(payment){
					...
					for(var i = 0; i < fees.length; i++){
						let fee = fees[i];
						if(fee.code_name != "gfd"){
							totalPenalty = totalPenalty + fee.amount
							Payments
								.update({"_id": id, "fee":{ $elemMatch: {"_id": fee._id}}}, {
									$set: {
										"fee.$.needed_refund": true,
										"fee.$.updated_at": new Date(),
										"remarks": body.reason,
				            	...
				            	if(err){
				            		...
				            		resolve({message: "penalty", total_penalty: totalPenalty});
				            	}
				            });
						}
					}
			...
			.exec((err, agreement) => {
				...
				if(agreement){
					...
					let data = {
						...
						"type": paymentType,
						"attachment": {
							"payment": body.attachment
						},
						"status": "pending"
					};
					Payments.createPayments(data).then((res) => {
						var paymentId = res._id;
						if (type ==  "tenancy_agreement"){
...
agreementsSchema.static('paymentConfirmedLoi', (id:string, statusPay:string):Promise<any> => {
	return new Promise((resolve:Function, reject:Function) => {
		if (!_.isString(id)) {
			return reject(new TypeError('Id is not a valid string.'));
		}
		...
		Agreements
			...
			.exec((err, agreement) => {
				if(err){
					...
					agreement.letter_of_intent.data.status = statusPay;
					agreement.save((err, saved) => {
						if(err){
							reject({message: err.message});
						}
...
agreementsSchema.static('updateFee', (id:string, code:string, receiveAmount:number, needRefund:boolean, payment_confirm:string, status:string, remarks:string):Promise<any> => {
	return new Promise((resolve:Function, reject:Function) => {
		Payments
...
agreementsSchema.static('updateReceivePayment', (data:Object):Promise<any> => {
	return new Promise((resolve:Function, reject:Function) => {
		...
		Agreements.updateFee()

		//receive amount Std
		if(body.receiveAmountStd){
			Agreements.updateFee(body.paymentID, "std", body.receiveAmountStd, neededRefundGfd, body.payment_confirm, body.status, body.remarks);
		}

		//receive amount Gfd
		if(body.receiveAmountGfd){
			Agreements.updateFee(body.paymentID, "gfd", body.receiveAmountGfd, neededRefundStd, body.payment_confirm, body.status, body.remarks);			
		}

		//receive amount Scd
		if(body.receiveAmountScd){
			Agreements.updateFee(body.paymentID, "scd", body.receiveAmountScd, neededRefundScd, body.payment_confirm, body.status, body.remarks);
		}
						...
						if(taData.status == "draft"){
							...
							Agreements.paymentCekStatus(paymentLoiID).then((res) => {
								if(res.message){
									...
									if(body.status == "rejected"){
										Agreements.penalty(paymentLoiID, data);
										Agreements.penalty(paymentTaID, data);
										// Agreements.updateReceivePayment(data);
										Agreements.notification(id, type_notif);										
										Agreements.email(id, typeMail);
									}
...
agreementsSchema.static('refundPenaltyPayment', (idPayment:string, idfee:string):Promise<any> => {
	return new Promise((resolve:Function, reject:Function) => {
		Payments
		...
		Agreements
			...
			.exec((err, res) => {
				...
				if(res){
					...
					let paymentTaStatus = res.tenancy_agreement.data.status;
					if(paymentTaStatus == "admin-confirmation"){
						Payments
							.findById(paymentLoi)
							...
							.exec((err, payLoiStd) => {
								...
								if(payLoiStd){
									let feeStd = payLoiStd.fee;
									for(var i = 0; i < feeStd.length; i++){
										let receivedAmountStd = feeStd[i].received_amount;
										let idStd = feeStd[i]._id;
										Agreements.refundPenaltyPayment(paymentLoi, idStd);															
										feeRefund = receivedAmountStd;
									}
									Payments
										.findById(paymentLoi)
										.select({"fee": {$elemMatch: {"code_name": "mmr"}}})
										.exec((err, payLoiMmr) => {
											if(err){
												reject({message: err.message});
											}
											if(payLoiMmr){
												if(payLoiMmr){
													let feeMmr = payLoiMmr.fee;
													for(var i = 0; i < feeMmr.length; i++){
														let receivedAmountMmr = feeMmr[i].received_amount;
														let idMmr = feeMmr[i]._id;
														Agreements.refundPenaltyPayment(paymentLoi, idMmr);
														feeRefund = feeRefund + receivedAmountMmr;
													}					
												}
												Payments
													.findById(paymentTa)
													...
													.exec((err, payTaScd) => {
														...
														if(payTaScd){
															if(payTaScd){
																let feeScdTa = payTaScd.fee;
																for(var i = 0; i < feeScdTa.length; i++){
																	let receivedAmountScd = feeScdTa[i].received_amount;
																	let idScd = feeScdTa[i]._id;
																	Agreements.refundPenaltyPayment(paymentLoi, idScd);
																	feeRefund = feeRefund + receivedAmountScd;
																}					
															}
															resolve({message: "penalty", "total refund": feeRefund});												
														}	
													})											
											}	
										})				
